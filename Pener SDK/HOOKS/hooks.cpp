...
namespace HOOKS
{
	...
	bool __stdcall HookedCreateMove(float sample_input_frametime, SDK::CUserCmd* cmd)
	{
		...
		PVOID pebp;
		__asm mov pebp, ebp;
		bool* SendPacket = (bool*)(*(DWORD*)pebp - 0x1C);
		...
		SDK::ConVar* var = mopvar::cvar->FindVar("weapon_debug_spread_show");
...
#define XorStr( s ) ( s )
				switch (int(mopvar::Globals->curtime * 2.4) % 2)
				{
				case 0: setclantag(XorStr("+ stephack.club -")); break;
				case 2: setclantag(XorStr("- stephack.club +")); break;
	...
	void __fastcall HookedSceneEnd(void* ecx, void* edx)
	{
		...
		if (mopvar::Engine->IsConnected( ) && mopvar::Engine->IsInGame( ))
		{
			for (int i = 1; i < 65; i++)
			{
				...
				if (!is_teammate)
					continue;
				if (!SETTINGS::settings.localplayerchams_bool) {
					if (!local_player)
						continue;
				}
			...
			*(int*)(smokecout) = 0;
			...
			for (int i = 1; i < 65; i++)
			{
				...
				{
					mopvar::RenderView->SetBlend(0.2);
				}
			}
		}
	}
