...
void CVisuals::Draw()
{
	for (int i = 1; i < 65; i++)
	{
	    ...
	    if (SETTINGS::settings.povcircles_bool)	DrawDirection(entity->GetVecOrigin());
		//DrawHitbox(entity);
...
void CVisuals::ClientDraw()
{
	...
	if (SETTINGS::settings.recoil_circle)
	{
		...
		RENDER::DrawFilledCircle(xs, ys, accuracy, 60, CColor(255, 105, 108, 124));
...
void CVisuals::DrawIndicator()
{
	...
	int W, H, cW, cH;
	mopvar::Engine->GetScreenSize(W, H);
	cW = W / 2;
	cH = H / 2;
	if (SETTINGS::settings.manualaa_bool)
	{
		...
		if (SETTINGS::settings.aa_type == 2 || SETTINGS::settings.aa_type == 4)
		{
			...
			{
				RENDER::DrawWF(cW + 34, cH - 20, FONTS::visuals_side_font, CColor(10, 145, 190, 130), L"▶"); //green
				RENDER::DrawWF(cW - 50, cH - 20, FONTS::visuals_side_font, CColor(255, 255, 255, 130), L"◀");
			}
			else
			{
				RENDER::DrawWF(cW + 34, cH - 20, FONTS::visuals_side_font, CColor(255, 255, 255, 130), L"▶"); //green
				RENDER::DrawWF(cW - 50, cH - 20, FONTS::visuals_side_font, CColor(10, 145, 190, 130), L"◀");
			}
...
void CVisuals::DrawDirection(const Vector& origin)
{
...
void CVisuals::ModulateWorld() //credits to my nigga monarch
{
	...
	if (!nightmode_performed)
	{
		static auto r_DrawSpecificStaticProp = mopvar::cvar->FindVar("r_DrawSpecificStaticProp");
		r_DrawSpecificStaticProp->nFlags &= ~FCVAR_CHEAT;
		r_DrawSpecificStaticProp->SetValue(1);

		static auto sv_skyname = mopvar::cvar->FindVar("sv_skyname");
		sv_skyname->nFlags &= ~FCVAR_CHEAT;

		static auto mat_postprocess_enable = mopvar::cvar->FindVar("mat_postprocess_enable");
		...
		for (SDK::MaterialHandle_t i = mopvar::MaterialSystem->FirstMaterial(); i != mopvar::MaterialSystem->InvalidMaterial(); i = mopvar::MaterialSystem->NextMaterial(i))
		{
			...
			else if (strstr(pMaterial->GetTextureGroupName(), "StaticProp"))
			{
				if (SETTINGS::settings.night_bool)
				{
					pMaterial->ColorModulate(0.10, 0.10, 0.60);
...
void CVisuals::DrawHitmarker()
{
	...
	for (int i = 0; i < 10; i++) { //pixel by pixel method
		...
		RENDER::DrawLine(center_width / 2 - 10, center_height / 2 - 10, center_width / 2 - 5, center_height / 2 - 5, CColor(255, 255, 255, hitmarker_time));
		RENDER::DrawLine(center_width / 2 - 10, center_height / 2 + 10, center_width / 2 - 5, center_height / 2 + 5, CColor(255, 255, 255, hitmarker_time));
		RENDER::DrawLine(center_width / 2 + 10, center_height / 2 - 10, center_width / 2 + 5, center_height / 2 - 5, CColor(255, 255, 255, hitmarker_time));
		RENDER::DrawLine(center_width / 2 + 10, center_height / 2 + 10, center_width / 2 + 5, center_height / 2 + 5, CColor(255, 255, 255, hitmarker_time));
		}
